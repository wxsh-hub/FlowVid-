...
class CreateConfigRequest(BaseModel):
    ...
    text_protocol: Optional[str] = "openai"  # openai / dashscope / custom
    ...
    image_protocol: Optional[str] = "doubao"  # doubao / openai / dashscope / custom
...
async def update_config(config_id: str, request: CreateConfigRequest, db: Session = Depends(get_db)):
    ...
    # 如果API Key是占位符，则保持原值不变
    if request.text_api_key and request.text_api_key != '**********':
    ...
    # 如果API Key是占位符，则保持原值不变
    if request.image_api_key and request.image_api_key != '**********':
